...
export function makeServer({ environment = "development" } = {}) {
  let server = createServer({
    ...
    seeds(server) {
      let admin = server.create("user", { username: "admin", role: 'admin', password: 'admin', enrolledCourses: [] })
      let bob = server.create("user", { username: "Bob", role: "user", password: 'bob', enrolledCourses: [] })
      let alice = server.create("user", { username: "Alice", role: "user", password: 'alice', enrolledCourses: [] })
      let project_one = server.create("course", { title: "Vuex Course", description: 'Time to learn some Vuex', isPublic: true , owner: alice, enrolledUsers: [bob, admin], imageUrl: 'https://vuex.vuejs.org/flow.png' })
      ...
      server.create('lecture', { title: "Lecture 1", video_url: 'a video url', course: project_one })
      server.create('lecture', { title: "Lecture 2", video_url: 'a video url', course: project_one })
    ...
    routes() {
      ...
      this.get("/courses/:id/lectures", (db, request) => {
        return db.lectures.where({courseId: request.params.id})
      })
